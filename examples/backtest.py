...
def load_signals(filepath):
    """Load signals with price journal data from JSONL file."""
...
def net_pnl(buy_size_usd, gross_mult, sol_price, num_sell_txs=1):
    """
    ...
    """
    fixed_tx_cost = (JITO_TIP_SOL + PRIORITY_FEE_SOL) * sol_price
    effective_buy = buy_size_usd * (1 - BUY_COST_PCT)
    gross_value = effective_buy * gross_mult
    after_sell_pct = gross_value * (1 - SELL_COST_PCT)
    after_fixed = after_sell_pct - (fixed_tx_cost * num_sell_txs) - fixed_tx_cost
    return after_fixed - buy_size_usd


def run_sweep(signals, buy_size=50.0, sol_price=130.0):
    """
    ...
    for tp_mult in [1.3, 1.5, 2.0, 2.5, 3.0]:
        for fallback in ["15m", "30m", "1hr"]:
            pnls = [
                net_pnl(buy_size, sim_tp_with_fallback(s, tp_mult, fallback), sol_price)
                for s in signals
            ]
...
if __name__ == "__main__":
    ...
    # Apply quality filters — holder growth and bot/degen rate
    # were the strongest predictors of returns in our analysis
    filtered = [s for s in signals if
        s.get("holder_growth_pct", 0) >= 10 and
        s.get("bot_degen_rate", 100) <= 0.40
    ]
    ...
    print("=" * 90)
    run_sweep(filtered, buy_size=50.0, sol_price=130.0)
